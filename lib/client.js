	...
	factory: (require) => {
		...
		function createOverlay(ctx) {
			...
			return function FileOpenWithOverlay() {
				...
				const run = react.useCallback(async (id, rawPath) => {
					const absolute = absolutize(rawPath);
					if (absolute === null) {
				...
				const items = buildItems(apps, menu.path);
				...
				const menuElement = usesPrimitives
					? react.createElement(primitives.Menu, {
						...
						onSelect: (id) => { setMenu(null); void run(id, menu.path); },
